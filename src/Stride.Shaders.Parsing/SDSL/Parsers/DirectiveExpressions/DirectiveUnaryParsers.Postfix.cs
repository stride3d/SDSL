        ...
        where TScanner : struct, IScanner
    {
        ...
        if (PostfixParser.Indexer(ref scanner, result, out var expression))
        {
            ...
            if (
                Tokens.Char('.', ref scanner, advance: true)
                && CommonParsers.Spaces0(ref scanner, result, out _)
                && PostfixParser.Accessor(ref scanner, result, out var accessed, new(SDSLErrorMessages.SDSL0021, scanner[scanner.Position], scanner.Memory)))
            {
        ...
        where TScanner : struct, IScanner
    {
        ...
        if (PrimaryParsers.Primary(ref scanner, result, out var expression))
        {
            ...
            if (Tokens.Char('[', ref scanner, advance: true))
            {
                if (
                    CommonParsers.Spaces0(ref scanner, result, out _)
                    && ExpressionParser.Expression(ref scanner, result, out var index, new(SDSLErrorMessages.SDSL0010, scanner[scanner.Position], scanner.Memory))
                    && CommonParsers.Spaces0(ref scanner, result, out _)
                    && Tokens.Char(']', ref scanner, advance: true)
                )
                {
                ...
                {
                    result.Errors.Add(new(SDSLErrorMessages.SDSL0021, scanner[position], scanner.Memory));
                    parsed = null!;
                    return false;
                }
            }
